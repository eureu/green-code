    ...
    const fetchCreatures = async () => {
      try {
        const response = await fetch(
          "http://194.58.98.44:8000/api/v1/get_creatures",
          {
            method: "POST",
            headers: {
    ...
    <>
      ...
      <div className={style.mainPage}>
        <div>
          ...
          <span>
            <span className={style.header}>Животные</span>
          ...
          <div className={style.creature_grid_}>
            ...
            {creaturesData?.data.length === 0 && (
              <span className={style.notFound}>Растения не найдены</span>
            )}
          </div>
        </div>
        <div>
